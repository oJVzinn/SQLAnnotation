...
public class SQLUtils {
    ...
    public static List<Field> listFieldColumns(Class<?> tableClass) {
        Field[] fields = tableClass.getDeclaredFields();
        if (fields.length == 0) {
            throw new RuntimeException("To create a table it is necessary to have at least one column field");
        }

        return Arrays.stream(fields).filter(field -> field.getAnnotation(Column.class) != null).collect(Collectors.toList());
    }

    public static Field findPrimaryKey(Class<?> tableClass) {
    ...
    public static <T> T loadClass(Class<T> tableClass, Map<String, Object> values) {
        T instance;
        try {
