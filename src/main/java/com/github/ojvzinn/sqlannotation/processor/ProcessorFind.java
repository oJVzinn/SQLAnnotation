...
public class ProcessorFind implements Processor {
    ...
    public Object process(Method method, Object[] args, Class<?> entity) {
        ...
        LimitModel limit = extractLimitModel(args);
        switch (findType) {
            case BY_KEY: {
                ...
                return SQLAnnotation.getConfig().getSQLDataBase().getSelectModule().findByConditionals(entity, joinModel, conditional, order, limit);
            }

            case ALL: {
                return SQLAnnotation.getConfig().getSQLDataBase().getSelectModule().findAll(entity, order, limit);
            }
            ...
            case BY_CONDITIONALS: {
                if (args[0] instanceof ConditionalModel)
                    return SQLAnnotation.getConfig().getSQLDataBase()
                            .getSelectModule()
                            .findResult(entity, joinModel, (ConditionalModel) args[0], order, limit);
                ...
                return SQLAnnotation.getConfig().getSQLDataBase().getSelectModule().findResult(entity, joinModel, conditional, order, limit);
            }
        }
    ...
    private LimitModel extractLimitModel(Object[] args) {
        LimitModel limit = null;
        for (Object arg : args)
            if (arg instanceof LimitModel) {
                limit = (LimitModel) arg;
                break;
            }
